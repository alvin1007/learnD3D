...
bool SystemClass::Initialize()
{
    ...
    // m_Timer 객체 생성. 프레임 개수를 구하기 위한 객체입니다.
    m_Timer = new GameTimerClass;
    if (!m_Timer)
    {
        return false;
    }
...
void SystemClass::CalculateFrameStats()
{
    ...
    if ((m_Timer->TotalTime() - timeElapsed) >= 1.0f)
    {
        ...
        outs.precision(6);
        outs << m_applicationName << L"    " << L"FPS: " << fps << L"   " << L"Frame Time: " << mspf << L" (ms)";
...
void SystemClass::InitializeWindows(int& screenWidth, int& screenHeight)
{
    ...
    // 프로그램 이름을 지정합니다
    m_applicationName = L"DirectX 11";
    ...
    // 윈도우를 생성하고 핸들을 구합니다.
    m_hwnd = CreateWindowEx(WS_EX_APPWINDOW, m_applicationName, m_applicationName, WS_EX_OVERLAPPEDWINDOW,
